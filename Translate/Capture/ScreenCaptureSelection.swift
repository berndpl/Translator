...
class ScreenCaptureSelection {
    ...
    /// Cancel the current selection (called when modifiers are released)
    func cancelSelection() {
        queue.async { [weak self] in
            guard let self = self, self.isSelectionActive else { return }
    ...
    private func handleSelection(rect: CGRect, window: NSWindow) {
        queue.async { [weak self] in
            ...
            self.selectionWindow = nil
            self.isSelectionActive = false // Mark selection as completed
